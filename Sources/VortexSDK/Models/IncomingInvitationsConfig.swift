...
public struct IncomingInvitationsConfig {
    ...
    /// invitation's metadata (e.g., a user handle stored in `metadata["inviter_handle"]`).
    ///
    /// If not provided, no subtitle is rendered.
    public let getSubtitle: ((IncomingInvitationItem) -> String?)?
    ...
    public init(
        ...
        getSubtitle: ((IncomingInvitationItem) -> String?)? = nil
    ) {
